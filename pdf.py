...
def pdf(sales, top10_searches, bottom10_searches, top5_scores, bottom5_scores, meses, anuales, mes):
  ...
  pag_1, ((tab1, ax1), (tab2, ax2), (tab3, ax3)) = plt.subplots(3, 2, figsize=(9, 11), gridspec_kw={'width_ratios': [2, 1]})
  tab = [tab1, tab2, tab3]
  ax = [ax1, ax2, ax3]
  ...
  for count, mes_anio in enumerate(sales['Mes_anio'].sort_values().unique()[-3:]):
    ...
    tab[count].table(cellText=top5_sales.values, colLabels=top5_sales.columns, rowLabels=top5_sales.index.str[:23], loc='center',
  ...
  tab.table(cellText=top10_searches.values, colLabels=top10_searches.columns, rowLabels=top10_searches.index.str[:23], loc='center', colColours=np.full(len(top10_searches.columns), fill_value='#d9d9d9'))
  tab.axis('off')
  ...
  pag_3, ((tab1, ax1), (tab2, ax2), (tab3, ax3)) = plt.subplots(3, 2, figsize=(9, 11), gridspec_kw={'width_ratios': [2, 1]})
  tab = [tab1, tab2, tab3]
  ax = [ax1, ax2, ax3]
  ...
  for count, mes_anio in enumerate(sales['Mes_anio'].sort_values().unique()[-3:]):
    ...
    tab[count].table(cellText=bottom5_sales.values, colLabels=bottom5_sales.columns, rowLabels=bottom5_sales.index.str[:23], loc='center',
  ...
  tab.table(cellText=bottom10_searches.values, colLabels=bottom10_searches.columns, rowLabels=bottom10_searches.index.str[:23], loc='center', colColours=np.full(len(bottom10_searches.columns), fill_value='#d9d9d9'))
  tab.axis('off')
  ...
  tab1.table(cellText=top5_scores.values, colLabels=top5_scores.columns, rowLabels=top5_scores.index.str[:23], loc='center', colColours=np.full(len(top5_scores.columns), fill_value='#d9d9d9'))
  tab1.axis('off')
